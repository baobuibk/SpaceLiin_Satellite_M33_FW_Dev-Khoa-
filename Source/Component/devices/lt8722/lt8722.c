...
{
	SP_GPIO_PinWrite(dev->cs_port, dev->cs_pin, 0);
}

static inline void csHIGH(struct lt8722_dev *dev)
{
	SP_GPIO_PinWrite(dev->cs_port, dev->cs_pin, 1);
}
...
//	LL_GPIO_ResetOutputPin((GPIO_TypeDef*)swen_port[channel], swen_pin[channel]);
	SP_GPIO_PinWrite(dev->en_port, dev->en_pin, 0);
	SP_GPIO_PinWrite(dev->swen_port, dev->swen_pin, 0);
...
//	LL_GPIO_SetOutputPin((GPIO_TypeDef*)en_port[channel], en_pin[channel]);
	SP_GPIO_PinWrite(dev->en_port, dev->en_pin, 1);
