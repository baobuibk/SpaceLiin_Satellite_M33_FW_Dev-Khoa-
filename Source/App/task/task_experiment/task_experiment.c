...
static uint8_t    DAC_code_from_percent = 0;
...
extern uint8_t  is_spi_counter_finish;
...
void Task_Experiment(void *pvParameters)
{
    ...
    {
    // SET_UP_STATE:
        system_data_get_is_start_exp(&is_start_exp);

        while (is_start_exp == 0)
        {
            TickType_t wait_start = pdMS_TO_TICKS(100);
            vTaskDelayUntil(&exp_last_delay, wait_start);

            // Update timing data
            system_data_get_is_start_exp(&is_start_exp);
        }

        system_data_update_is_start_exp(0);
        ...
        bsp_debug_console_printf("PRE %d, SAM %d, POST %d\n> ", pre_delay, sample_delay, post_delay);
        ...
        {
    // PRE_TIME_STATE:
            bsp_debug_console_printf("CHANNEL: %d\n> ", current_channel + 1);
            bsp_debug_console_send_string("PRE_TIME_STATE\n> ");
    ...
    // SAMPLING_STATE:
            bsp_debug_console_send_string("SAMPLING_STATE\n> ");
            bsp_laser_int_set_dac(DAC_code_from_percent);

            vTaskDelayUntil(&exp_last_delay, sample_delay);

    // POST_TIME_STATE:
            bsp_debug_console_send_string("POST_TIME_STATE\n> ");
            ...
            while (is_spi_counter_finish == 0)
            {
                ...
                if(wait_spi_timeout == 0)
                {
                    ...
                    // bsp_photo_spi_irq_init();
                    bsp_debug_console_printf("CHANNEL %d ERROR\n> ", current_channel + 1);
