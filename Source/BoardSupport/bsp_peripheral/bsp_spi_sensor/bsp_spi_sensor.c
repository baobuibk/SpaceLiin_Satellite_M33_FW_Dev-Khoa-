...
void bsp_spi_sensor_read_value(Sensor_Read_typedef read_type)
{
...
static uint8_t BMP390_Write(uint8_t reg, uint8_t TX_data)
{
    ...
    cs_low();
    SP_SPI_master_write_blocking(bmp390_device.p_spi, &spi_frame);
    cs_high();
...
static uint8_t BMP390_Read(uint8_t reg, uint8_t *p_RX_buffer, uint8_t byte_count)
{
    ...
    cs_low();
    SP_SPI_master_read_blocking(bmp390_device.p_spi, &spi_frame);
    cs_high();
