...
void bsp_spi_sensor_init()
{
	...
	photo_adc_gpio_handle.p_base = PHOTO_ADC_GPIO_PORT;

    photo_adc_spi_device.p_spi   = &photo_adc_spi_handle;
    photo_adc_spi_device.cs_port = &photo_adc_gpio_handle;
    photo_adc_spi_device.cs_pin  = PHOTO_ADC_GPIO_SPI_CS_PIN;
...
void PHOTO_ADC_TPM_HANDLER(void)
{
	...
	PHOTO_ADC_GPIO_PORT->PCOR = 1UL << PHOTO_ADC_GPIO_SPI_CV_PIN;

    //	GPIOD->BSRR = GPIO_BSRR_BS_9;  	// CS HIGH
	PHOTO_ADC_GPIO_PORT->PSOR = 1UL << PHOTO_ADC_GPIO_SPI_CS_PIN;

    //	GPIOD->BSRR = GPIO_BSRR_BS_10; 	// CV HIGH
	PHOTO_ADC_GPIO_PORT->PSOR = 1UL << PHOTO_ADC_GPIO_SPI_CV_PIN;
