...
flexcan_frame_t LIBCSP_CAN_TX_buffer[TX_BUFFER_SIZE];

ring_buffer_t   LIBCSP_CAN_RX_ring_buffer;
flexcan_frame_t LIBCSP_CAN_RX_buffer[RX_BUFFER_SIZE];
...
void bsp_libcsp_can_init()
{
    ...
    RX_frame.id     = FLEXCAN_ID_STD(RX_IDENTIFIER);
    ...
    ring_buffer_init(&LIBCSP_CAN_TX_ring_buffer, (uint8_t*)LIBCSP_CAN_TX_buffer, sizeof(LIBCSP_CAN_TX_buffer), TX_BUFFER_SIZE, sizeof(flexcan_frame_t));
    ring_buffer_init(&LIBCSP_CAN_RX_ring_buffer, (uint8_t*)LIBCSP_CAN_RX_buffer, sizeof(LIBCSP_CAN_RX_buffer), RX_BUFFER_SIZE, sizeof(flexcan_frame_t));
