 ...
 * This function is useful for generating CSV files for telemetry or data analysis.
 * @param stream Stream
 * @param data Payload struct
 * @param payload Payload type
 * @return Returns 0 if the operation is successful, -1 if an error occurs (e.g., if the stream is NULL).
 */
int dat_fprint_payload_struct(FILE *stream, void* data, unsigned int payload);


/**
 * Writes the header of a payload structure to a given stream in a comma-separated format.
 * This function is useful for generating CSV files for telemetry or data analysis.
 *
 * @param stream The output stream where the header will be written.
 * @param payload The payload ID whose header will be printed.
 * @return Returns 0 if the operation is successful, -1 if an error occurs (e.g., if the stream is NULL).
 */
int dat_fprint_payload_header(FILE *stream, unsigned int payload);
