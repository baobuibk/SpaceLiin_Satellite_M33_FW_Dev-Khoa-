/*
 * Copyright 2025 SUCHAI Flight Software v2 project and contributors
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     https://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef CMD_REPO_H
#define CMD_REPO_H

#include <stdarg.h>

#include "log_utils.h"
#include "globals.h"
#include "storage.h"


/* Macros */
/**
 * Send command to execution using dispatcherQueue (must be initialized). Blocks
 * if the queue is full
 *
 * @param cmd *cmd_type, pointer to command
 */
/**
 *  Defines the prototype of a command
 */
#define cmd_send(cmd) if(&cmd != NULL){osQueueSend(dispatcher_queue, &cmd, portMAX_DELAY);}


/* Command definitions */

/**
 * Defines the arguments passed to a command.
 * Parameters are passed as a string, so extra information is provided to parse the parameters from the
 * string, such as the number of parameters and the parameters' format.
 */
typedef struct cmd_args {
    int32_t id; ///< Command id
    int32_t nargs; ///< Number of parameters
    uint8_t *argv[SCH_CMD_MAX_ARG + 1]; //An array to hold the pointers to the command line arguments.
//    char fmt[SCH_CMD_MAX_STR_FORMAT]; ///< Format of the parameters as a string
    uint8_t  args[SCH_BUFF_MAX_LEN]; ///< List of parameters as a string
} cmd_args_t;


/**
 * Defines the result format for a command.
 * It includes an error field and a result field. The user must check the error field before using the result field.
 * If the error field is other than 0, then it is assumed the command created a result value.
 */
typedef struct cmd_result {
	int32_t			id;
    int32_t 		error;
    value32_t 		result;
    uint32_t 		outputSize;
    uint8_t 		output[SCH_BUFF_MAX_LEN]	;
} cmd_result_t;


#define CMD_SRC_CONSOLE	0
#define CMD_SRC_CSP		1

typedef void (*cmdFunction)(uint32_t argc, uint8_t *argv[],  cmd_result_t * result);


/**
 * Structure to store a command sent to execution
 */
typedef struct cmd_type {
    int32_t id; ///< Command id
    uint32_t cmdSrc;
    cmd_args_t args; ///< Command arguments
    cmdFunction function; ///< Command function
} cmd_t;

/**
 * Structure to store the list of available commands by name
 */
typedef struct cmd_list_type {
    int32_t nargs; ///< Number of parameters
    char fmt[SCH_CMD_MAX_STR_FORMAT]; ///< Format of the parameters as a string
    char name[SCH_CMD_MAX_STR_NAME]; ///< Command name (use malloc)
    cmdFunction function; ///< Command function
} cmd_list_t;




typedef struct
{
	uint32_t    id;
    const char *pcCmd;
    cmdFunction pfnCmd;
    const char *pcHelp;
}cmd_table_entry_t;
/* Function definitions */

/**
 * Registers a command in the system
 *
 * @param name Parameter name
 * @param function Pointer to command function
 * @param fmt Str. defines format of parameters, separated by spaces
 * @param nargs Int. number of parameters, according to @fparams
 * @return Int. Current length of command list.
 *
 * @code
 *      // Adds command foo with 2 params: integer and string
 *      cmd_add("foo", foo, "%d %s", 2);
 *      // Adds command bar with 3 params: integers and double
 *      cmd_add("bar", bar, "&d %d %f", 3);
 * @endcode
 */
int32_t cmd_add(const char *name, cmdFunction function, const char *fmt, int32_t nargs);

/**
 * Create a new command by name
 *
 * @param name Str. Command name
 * @return cmd_t * Pointer to command structure already initialized. Null if
 *                 command does not exists. Use @cmd_free to free allocated
 *                 memory.
 */
cmd_t *cmd_get_str(const char *name);

/**
 * Create a new command by index or id
 *
 * @param idx Int command index or Id.
 * @return cmd_t Command structure. Null if command does not exist.
 */
cmd_t *cmd_get_idx(int idx);

/**
 * Find the name of a command by id.
 * This function returns a pointer to the string with the name of the command that msut not
 * be modified or freed.
 *
 * @param idx Int. Command index or id
 * @return Str. Command name.
 */
const char *cmd_get_name(int idx);

/**
 * Fills command parameters as raw data using memcpy. Exactly @len bytes will be copied
 * from @params to @cmd->args.args.
 *
 * @note make sure that @params contains at least @len bytes.
 *
 * @param cmd cmd_t *. Command to fill parameters
 * @param params void *. Pointer to the data buffer
 * @param len int. Length of the data buffer.
 *
 * @code
 *      char *data = {0,1,2,3,4,5};
 *      cmd_t *foo = cmd_get_str("foo");
 *      cmd_add_params_raw(foo, data, 6);
 * @endcode
 */
void cmd_add_params_raw(cmd_t* cmd, const void* params, size_t len);

/**
 * Fills command parameters as string
 * @note does not check the parameters format or if the command requires param.
 *
 * @param cmd cmd_t. Command to fill parameters
 * @param params Str. String with parameters
 *
 * @code
 *      //Fill two parameters to foo command
 *      cmd_t *foo = cmd_get_str("foo"); // foo.fmt is '%d %s'
 *      cmd_add_params(foo, "123 abc");
 *      //Fill three parameters to bar command
 *      cmd_t *bar = cmd_get_str("bar"); // bar.fmt is '%d %d %f'
 *      cmd_add_params(bar, "1 2 3.4");
 * @endcode
 */
void cmd_add_params_str(cmd_t *cmd, const char *params);

/**
 * Fills command parameters by variables using the registered parameters format.
 * @note variables are converted to string
 *
 * @param cmd cmd_t. Command to fill parameters
 * @param ... List of variables to fill as parameters
 *
 * @code
 *      //Fill two parameters to foo command
 *      int a = 123; char *b = "abc";
 *      cmd_t *foo = cmd_get_str("foo"); // foo.fmt is '%d %s'
 *      cmd_add_params_var(foo, a, b);
 *      //Fill three parameters to bar command
 *      int c = 1; int d = 2; float e = 3.4;
 *      cmd_t *bar = cmd_get_str("bar"); // bar.fmt is '%d %d %f'
 *      cmd_add_params_var(bar, c, d, e);
 * @endcode
 */
void cmd_add_params_var(cmd_t *cmd, ...);

/**
 * Returns a new command with parameters form a string with the format:
 * <command> [parameters]. The [parameters] field is optional. Returns NULL if
 * the command is not found or in case of errors.
 *
 * @param buff str. A null terminated string with the format <command> [parameters]
 * @return cmd_t. A new command (uses malloc) or NULL in case of errors.
 *
 * @code
 *      char *str_cmd = "obc_debug 1";
 *      cmd_t *cmd = cmd_parse_from_str(str_cmd);
 *      assert(cmd != NULL);
 * @endcode
 */
uint32_t cmd_build_from_str(uint8_t * buffer, cmd_t * command);

/**
 * Destroys a command and frees the allocated memory
 */
void cmd_free(cmd_t* cmd);

/**
* Print the list of registered commands
*/
void cmd_print_all(void);

/**
 * Create a string with all available commands and their parameters
 * @return char*. List of commands. The string is allocated with calloc, so the user must free it after use.
 */
char* cmd_save_all(void);

/**
 * Initializes the command buffer adding null_cmd
 *
 * @return 1
 */
int cmd_repo_init(void);

/**
 * Cleans the repo buffer. Frees allocated memories
 */
void cmd_repo_close(void);

/**
 * Null command, just print to stdout
 *
 * @params fmt Not used
 * @params param Not used
 * @params nparams Not Used
 * @return 1, allways successful
 */
cmd_result_t cmd_null(cmd_args_t* params);

/**
 *
 *Print each value of the structure
 *
 * @param cmd cmd_t. The command that will be printed
 * @return 0 OK, -1 Error
 */
int cmd_print(const cmd_t* cmd);

uint32_t repo_command_static_find_command(const uint8_t * function, cmd_t * cmd);


extern cmd_table_entry_t g_psCmdTable[] ;
#endif /* CMD_REPO_H */
