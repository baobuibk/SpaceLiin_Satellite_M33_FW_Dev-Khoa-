...
void dat_repo_init(void)
{
    ...
    char fs_db_file[sizeof(SCH_STORAGE_FILE) + 10] = {0};  // Use sizeof on a string literal to make it constant
...
int dat_delete_payload(int payload)
{
    ...
    osSemaphoreTake(&repo_data_sem, portMAX_DELAY);
    //Enter critical zone
    int rc = storage_payload_reset_table(payload, data_map);
    //Exit critical zone
    osSemaphoreGiven(&repo_data_sem);
...
//    osSemaphoreTake(&repo_data_sem, portMAX_DELAY);
//    //Enter critical zone
//    ret = storage_payload_reset(data_map);
...
int dat_fprint_payload_header(FILE *stream, unsigned int payload)
{
    ...
    char *header = strdup(data_map[payload].var_names);
    for(int i=0; i<strlen(header); i++)
    {
        if(header[i] == ' ')
            header[i]=','; // Replace spaces with ','
    }
    fprintf(stream, "%s,\n", header);
    free(header);
    return 0;
